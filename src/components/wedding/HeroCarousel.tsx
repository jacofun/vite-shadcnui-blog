...
const slidesFromFolder = heroImageNames.map((name) => ({
  src: `/wedding-assets/hero/${name}-1080.webp`,
  srcSet: [640, 1080, 1440]
    .map((width) => `/wedding-assets/hero/${name}-${width}.webp ${width}w`)
    .join(", "),
}));
...
export default function HeroCarousel({
  ...
  const { likedCount, liked, loading, like } = useWalineLike({
    serverURL: "https://waline.yanxiao.me",
  ...
  return (
    <section
      className={cn(
        "relative h-[85svh] w-full overflow-hidden select-none",
        className
      )}
