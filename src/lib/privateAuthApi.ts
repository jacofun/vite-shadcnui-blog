...
export async function readAssessmentEventStream<T>(
  response: Response,
  ...
  if (!response.ok) {
    ...
    throw createError(
      ...
      payload.message ?? "AI 批改暂时不可用",
    );
  }
  if (!response.body) {
    throw createError(response.status, "INVALID_RESPONSE", "AI 批改服务未返回响应流");
  }
  ...
  function dispatch(block: string): void {
    ...
    if (event === "progress" && typeof payload.receivedChars === "number" && Number.isFinite(payload.receivedChars)) {
      onProgress(Math.max(0, payload.receivedChars));
    } else if (event === "result" && payload.result !== undefined) {
  ...
  try {
    ...
    if (!completed || result === undefined) {
      throw createError(response.status, "INCOMPLETE_STREAM", "AI 批改响应流意外中断");
    }
    return result;
  } finally {
